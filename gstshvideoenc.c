		...
		GstshvideoEncClass * gklass)
{
	...
	gst_pad_set_setcaps_function (shvideoenc->sinkpad, 
				      gst_sh_video_enc_setcaps);
	gst_pad_set_activate_function (shvideoenc->sinkpad, 
				       gst_sh_video_enc_activate);
	gst_pad_set_activatepull_function (shvideoenc->sinkpad,
					   gst_sh_video_enc_activate_pull);
	gst_pad_set_event_function(shvideoenc->sinkpad, 
				   gst_sh_video_enc_sink_event);
	gst_pad_set_chain_function(shvideoenc->sinkpad, 
		...
		const GValue * value, GParamSpec * pspec)
{
	...
	switch (prop_id) 
	{
		case PROP_CNTL_FILE:
		{
			strcpy(shvideoenc->ainfo.ctrl_file_name_buf,
...
gst_sh_video_enc_chain (GstPad * pad, GstBuffer * buffer)
{
	...
	memcpy(GST_BUFFER_DATA(enc->buffer_yuv),GST_BUFFER_DATA(buffer),
		yuv_size);
	...
	else
	{
		memcpy(GST_BUFFER_DATA(enc->buffer_cbcr),
		       GST_BUFFER_DATA(buffer)+yuv_size,
	 	       cbcr_size);
	}
...
gst_sh_video_enc_get_input(SHCodecs_Encoder * encoder, void *user_data)
{
	...
	{
		GST_DEBUG_OBJECT(shvideoenc,
				 "Encoding stop requested, returning 1");
		ret = 1;
	}
			...
			unsigned char *data, int length, void *user_data)
{
	...
	else if(length)
	{
		...
		GST_BUFFER_TIMESTAMP(buf) = 
			enc->frame_number*GST_BUFFER_DURATION(buf);
		enc->frame_number++;
