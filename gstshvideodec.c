...
gst_shvideodec_get_type (void)
{
  ...
  if (object_type == 0) 
  {
    ...
    object_type =
      g_type_register_static (GST_TYPE_ELEMENT, "gst-sh-mobile-dec", &object_info,
			      (GTypeFlags) 0);
  }
  return object_type;
}
...
gst_shvideodec_class_init (GstshvideodecClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_HW_BUFFER,
      g_param_spec_string ("hw-buffer", "HW-buffer", "Sets usage of HW buffers (auto(default)/yes/no)",
...
gst_shvideodec_setcaps (GstPad * pad, GstCaps * sink_caps)
{
  ...
  else
  {
    ...
    {
      GST_INFO_OBJECT(dec,"%s failed (not supported: %s)",__FUNCTION__,gst_structure_get_name (structure));
      return FALSE;
    }
  }
  ...
  if(gst_structure_get_fraction (structure, "framerate", 
				  ...
				  &dec->fps_denominator))
  {    
    GST_INFO_OBJECT(dec,"Framerate: %d/%d",dec->fps_numerator,dec->fps_denominator);
  }
  else
  {
  ...
  {
    GST_INFO_OBJECT(dec,"%s initializing decoder %dx%d",__FUNCTION__,dec->width,dec->height);
...
gst_shvideodec_decode (void *data)
{
  ...
  do
  {
    ...
    GST_DEBUG_OBJECT(dec,"Input buffer size: %d",GST_BUFFER_SIZE (buffer));
    ...
    {    
      ...
      if(dec->buffer)
      {
        dec->buffer = gst_buffer_join(
		       gst_buffer_create_sub(buffer,used_bytes,GST_BUFFER_SIZE(buffer)-used_bytes),
		       dec->buffer);
      }
      else
      {
        dec->buffer = gst_buffer_create_sub(buffer,used_bytes,GST_BUFFER_SIZE(buffer)-used_bytes);
      }
